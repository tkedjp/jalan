...
def add_json():
    global json_file
    file_type = [('json','*.json')]
    json_file = filedialog.askopenfilename(filetypes=file_type, initialdir='./')
    json_file_label.config(text='読み込み完了')
    json_file_label.update()
    print(json_file)
...
def save():
    ...
    sps_key = spreadsheet_key_entry.get()
    print(sps_key)
    ...
    total_number_tag = soup.select_one('div.jlnpc-planListCnt-header > span.jlnpc-listInformation--count')
    ...
    for i in range(max_page_index):
        ...
        page_soup = BeautifulSoup(page_r.content, 'html5lib')
        ...
        for table in table_soup:
            ...
            for i, page_url in enumerate(page_urls):  
                ...
                if 'javascript' in page_url:
                    ...
                    #ホテル名
                    hotel_page_soup = BeautifulSoup(hotel_page_r.content, 'html5lib')
...
#jsonファイル指定
json_file_add = tkinter.Button(text='json指定', command=add_json)
json_file_add.grid(row=5, column=1, padx=5, pady=5)
json_file_label = tkinter.Label(text='')
json_file_label.grid(row=5, column=2, padx=5, pady=5)

#スプレッドシートのキー入力
spreadsheet = tkinter.Label(text='シートID：')
spreadsheet.grid(row=6, column=1, padx=5, pady=5)
spreadsheet_key_entry = tkinter.Entry(text='')
spreadsheet_key_entry.grid(row=6, column=2, padx=5, pady=5)

#処理中
progress = tkinter.Label(text='処理状況：')
progress.grid(row=7, column=1, padx=5, pady=5)
progress_label = tkinter.Label(text='')
progress_label.grid(row=7, column=2, padx=5, pady=5)

#実行
save_button = tkinter.Button(text='取得', command=save)
save_button.grid(row=8, column=2, padx=5, pady=20, ipadx=4, ipady=4)
